...
unsigned __int64 RNGSobolBase::cjn[QRNG_NDMS][63] = { 0 }; ///< used for random numbers generation

template <bool UseGrayCode>
RNGSobolTmpl<UseGrayCode>::RNGSobolTmpl()
{
...
template <bool UseGrayCode>
void RNGSobolTmpl<UseGrayCode>::updateUValue()
{
	unsigned uTmp = UseGrayCode ? (m_uCurSeed >> 1) ^ m_uCurSeed : m_uCurSeed;
	unsigned changedBits = m_prev[m_uCurDim].uPrevSeed ^ uTmp;
	m_prev[m_uCurDim].uPrevSeed = uTmp;
	unsigned long uBit;
...
int RNGSobolBase::GeneratePolynomials(int buffer[QRNG_NDMS], bool primitive)
{
	int i, j, n, p1, p2, l;
	int e_p1, e_p2, e_b;
...
template class RNGSobolTmpl<true>;
template class RNGSobolTmpl<false>;